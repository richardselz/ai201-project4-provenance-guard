...
# It instructs the model to use the FULL 0-1 range decisively for clear-cut
# cases (rather than hedging near 0.2/0.8), reserving mid-range values for
# genuine ambiguity. This keeps the confident label bands reachable.
_SYSTEM_PROMPT = (
    ...
    "Use the FULL range and be decisive when the evidence is clear:\n"
    "- 0.90-1.00: obvious AI writing (generic, uniform, buzzword-laden, "
    "formulaic transitions like 'furthermore'/'it is important to note').\n"
    "- 0.00-0.10: obvious human writing (personal voice, casual, idiosyncratic, "
    "typos, irregular rhythm).\n"
    "- 0.30-0.70: genuinely mixed or hard-to-tell cases only.\n"
    "Reserve 0.5 for true coin-flip uncertainty. Do not cluster around 0.2/0.8 "
    "out of caution — commit to the extremes when the text clearly warrants it.\n"
    "IMPORTANT: formal, academic, technical, or non-native-English writing is "
    "very often HUMAN. Do not rate text as AI merely for being polished, "
    "impersonal, or grammatically formal. Judge AI-ness by formulaic emptiness "
    "and generic filler, not by sophistication. When a formal piece could "
    "plausibly be a human expert, prefer a mid-range score over a confident one."
)
...
def protective_adjustments(styl_result):
    ...
    if not styl_result.get("small_sample") and ttr < 0.55 and burstiness < 4.0:
        # Larger skew than small_sample: here both signals actively misfire on a
        # known human pattern (repetitive verse), so we pull harder toward human.
        adjustments.append(("simple_repetitive_vocabulary", -0.20))
